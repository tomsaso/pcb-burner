...
class ESPLoader(object):
    ...
    def detect_chip(port=DEFAULT_PORT, baud=ESP_ROM_BAUD, connect_mode='default_reset', trace_enabled=False):
        ...
        _print('Detecting chip type...', end='')
        sys.stdout.flush()
    ...
    def trace(self, message, *format_args):
        if self._trace_enabled:
            ...
            _print(prefix + (message % format_args))
    ...
    def flash_begin(self, size, offset):
        ...
        if size != 0 and not self.IS_STUB:
            _print("Took %.2fs to erase flash block" % (time.time() - t))
        return num_blocks
    ...
    def flash_defl_begin(self, size, compsize, offset):
        ...
        _print("Compressed %d bytes to %d..." % (size, compsize))
    ...
    def change_baud(self, baud):
        _print("Changing baud rate to %d" % baud)
...
class ESP32ROM(ESPLoader):
    ...
    def override_vddsdio(self, new_voltage):
        ...
        _print("VDDSDIO regulator set to %s" % new_voltage)
    ...
    def warn_if_unusual_segment(self, offset, size, is_irom_segment):
        if not is_irom_segment:
            if offset > 0x40200000 or offset < 0x3ffe0000 or size > 65536:
                _print('WARNING: Suspicious segment 0x%x, length %d' % (offset, size))
    ...
    def __init__(self, load_file=None):
        ...
        if load_file is not None:
            ...
            if segments != ESPBOOTLOADER.IMAGE_V2_SEGMENT:
                ...
                _print('Warning: V2 header has unexpected "segment" count %d (usually 4)' % segments)
            ...
            if first_flash_mode != self.flash_mode:
                _print('WARNING: Flash mode value in first header (0x%02x) disagrees with second (0x%02x). Using second value.'
                      % (first_flash_mode, self.flash_mode))
            if first_flash_size_freq != self.flash_size_freq:
                _print('WARNING: Flash size/freq value in first header (0x%02x) disagrees with second (0x%02x). Using second value.'
                      % (first_flash_size_freq, self.flash_size_freq))
            if first_entrypoint != self.entrypoint:
                _print('WARNING: Entrypoint address in first header (0x%08x) disagrees with second header (0x%08x). Using second value.'
                      % (first_entrypoint, self.entrypoint))
...
class ESP32FirmwareImage(BaseFirmwareImage):
    ...
    def load_extended_header(self, load_file):
        ...
        if any(f for f in fields[4:15] if f != 0):
            _print("Warning: some reserved header fields have non-zero values. This image may be from a newer esptool.py?")
...
class ELFFile(object):
    ...
    def _read_sections(self, f, section_header_offs, section_header_count, shstrndx):
        ...
        if sec_type != ELFFile.SEC_TYPE_STRTAB:
            _print('WARNING: ELF file has incorrect STRTAB section type 0x%02x' % sec_type)
        f.seek(sec_offs)
...
def load_ram(esp, args):
    ...
    for seg in image.segments:
        size = len(seg.data)
        _print('Downloading %d bytes at %08x...' % (size, seg.addr), end=' ')
        sys.stdout.flush()
    ...
    _print('All segments done, executing at %08x' % image.entrypoint)
    esp.mem_finish(image.entrypoint)


def read_mem(esp, args):
    _print('0x%08x = 0x%08x' % (args.address, esp.read_reg(args.address)))


def write_mem(esp, args):
    esp.write_reg(args.address, args.value, args.mask, 0)
    _print('Wrote %08x, mask %08x to %08x' % (args.value, args.mask, args.address))
...
def dump_mem(esp, args):
    ...
    for i in range(args.size // 4):
        ...
        if f.tell() % 1024 == 0:
            _print('\r%d bytes read... (%d %%)' % (f.tell(),
...
def detect_flash_size(esp, args):
    if args.flash_size == 'detect':
        ...
        if args.flash_size is None:
            _print('Warning: Could not auto-detect Flash size (FlashID=0x%x, SizeID=0x%x), defaulting to 4MB' % (flash_id, size_id))
            args.flash_size = '4MB'
        else:
            _print('Auto-detected Flash size:', args.flash_size)
...
def _update_image_flash_params(esp, address, args, image):
    ...
    if flash_params != image[2:4]:
        _print('Flash params set to 0x%04x' % struct.unpack(">H", flash_params))
...
def write_flash(esp, args):
    ...
    for address, argfile in args.addr_filename:
        if args.no_stub:
            _print('Erasing flash...')
        image = pad_to(argfile.read(), 4)
        if len(image) == 0:
            _print('WARNING: File %s is empty' % argfile.name)
            continue
        ...
        t = time.time()
        while len(image) > 0:
            _print('\rWriting at 0x%08x... (%d %%)' % (address + seq * esp.FLASH_WRITE_SIZE, 100 * (seq + 1) // blocks), end='')
            sys.stdout.flush()
        ...
        if args.compress:
            ...
            _print('\rWrote %d bytes (%d compressed) at 0x%08x in %.1f seconds%s...' % (uncsize, written, address, t, speed_msg))
        else:
            if t > 0.0:
                speed_msg = " (%.1f kbit/s)" % (written / t * 8 / 1000)
            _print('\rWrote %d bytes at 0x%08x in %.1f seconds%s...' % (written, address, t, speed_msg))
        try:
            res = esp.flash_md5sum(address, uncsize)
            if res != calcmd5:
                _print('File  md5: %s' % calcmd5)
                _print('Flash md5: %s' % res)
                _print('MD5 of 0xFF is %s' % (hashlib.md5(b'\xFF' * uncsize).hexdigest()))
                raise FatalError("MD5 of file does not match data in flash!")
            else:
                _print('Hash of data verified.')
    ...
    if args.verify:
        _print('Verifying just-written flash...')
        _print('(This option is deprecated, flash contents are now always read back after flashing.)')
        verify_flash(esp, args)


def image_info(args):
    image = LoadFirmwareImage(args.chip, args.filename)
    _print('Image version: %d' % image.version)
    _print('Entry point: %08x' % image.entrypoint if image.entrypoint != 0 else 'Entry point not set')
    _print('%d segments' % len(image.segments))
    print
    idx = 0
    for seg in image.segments:
        idx += 1
        _print('Segment %d: %r' % (idx, seg))
    calc_checksum = image.calculate_checksum()
    _print('Checksum: %02x (%s)' % (image.checksum,
    ...
    try:
        ...
        if image.append_digest:
            ...
            _print('Validation Hash: %s' % digest_msg)
    except AttributeError:
...
def elf2image(args):
    ...
    if args.chip == 'auto':  # Default to ESP8266 for backwards compatibility
        _print("Creating image for ESP8266...")
        args.chip = 'esp8266'
...
def read_mac(esp, args):
    ...
    def print_mac(label, mac):
        _print('%s: %s' % (label, ':'.join(map(lambda x: '%02x' % x, mac))))
    print_mac("MAC", mac)
...
def chip_id(esp, args):
    try:
        chipid = esp.chip_id()
        _print('Chip ID: 0x%08x' % chipid)
    except NotSupportedError:
        _print('Warning: %s has no Chip ID. Reading MAC instead.' % esp.CHIP_NAME)
        read_mac(esp, args)


def erase_flash(esp, args):
    _print('Erasing flash (this may take a while)...')
    t = time.time()
    esp.erase_flash()
    _print('Chip erase completed successfully in %.1fs' % (time.time() - t))


def erase_region(esp, args):
    _print('Erasing region (may be slow depending on size)...')
    t = time.time()
    esp.erase_region(args.address, args.size)
    _print('Erase completed successfully in %.1f seconds.' % (time.time() - t))
...
def flash_id(esp, args):
    flash_id = esp.flash_id()
    _print('Manufacturer: %02x' % (flash_id & 0xff))
    flid_lowbyte = (flash_id >> 16) & 0xFF
    _print('Device: %02x%02x' % ((flash_id >> 8) & 0xff, flid_lowbyte))
    _print('Detected flash size: %s' % (DETECTED_FLASH_SIZES.get(flid_lowbyte, "Unknown")))
...
def read_flash(esp, args):
    ...
    t = time.time() - t
    _print('\rRead %d bytes at 0x%x in %.1f seconds (%.1f kbit/s)...'
...
def verify_flash(esp, args):
    ...
    for address, argfile in args.addr_filename:
        ...
        _print('Verifying 0x%x (%d) bytes @ 0x%08x in flash against %s...' % (image_size, image_size, address, argfile.name))
        # Try digest first, only read if there are differences.
        digest = esp.flash_md5sum(address, image_size)
        expected_digest = hashlib.md5(image).hexdigest()
        if digest == expected_digest:
            _print('-- verify OK (digest matched)')
            continue
        else:
            differences = True
            if getattr(args, 'diff', 'no') != 'yes':
                _print('-- verify FAILED (digest mismatch)')
                continue

        flash = esp.read_flash(address, image_size)
        assert flash != image
        diff = [i for i in range(image_size) if flash[i] != image[i]]
        _print('-- verify FAILED: %d differences, first @ 0x%08x' % (len(diff), address + diff[0]))
        for d in diff:
            flash_byte = flash[d]
            image_byte = image[d]
            if PYTHON2:
            ...
            _print('   %08x %02x %02x' % (address + d, flash_byte, image_byte))
    if differences:
        raise FatalError("Verify failed.")


def read_flash_status(esp, args):
    _print('Status value: 0x%04x' % esp.read_status(args.bytes))


def write_flash_status(esp, args):
    fmt = "0x%%0%dx" % (args.bytes * 2)
    args.value = args.value & ((1 << (args.bytes * 8)) - 1)
    _print(('Initial flash status: ' + fmt) % esp.read_status(args.bytes))
    _print(('Setting flash status: ' + fmt) % args.value)
    esp.write_status(args.value, args.bytes, args.non_volatile)
    _print(('After flash status:   ' + fmt) % esp.read_status(args.bytes))
...
def main(_signal):
    ...
    _print('esptool.py v%s' % __version__)
    ...
    if operation_args[0] == 'esp':  # operation function takes an ESPLoader connection object
        ...
        if args.port is None:
            ...
            _print("Found %d serial ports" % len(ser_list))
        else:
            ser_list = [args.port]
        esp = None
        for each_port in reversed(ser_list):
            _print("Serial port %s" % each_port)
            try:
                if args.chip == 'auto':
                ...
                if args.port is not None:
                    raise
                _print("%s failed to connect: %s" % (each_port, err))
                esp = None
        if esp is None:
            raise FatalError("All of the %d available serial ports could not connect to a Espressif device." % len(ser_list))

        _print("Chip is %s" % (esp.get_chip_description()))

        _print("Features: %s" % ", ".join(esp.get_chip_features()))
        ...
        if args.baud > initial_baud:
            try:
                ...
                _print("WARNING: ROM doesn't support changing baud rate. Keeping initial baud rate %d" % initial_baud)

        # override common SPI flash parameter stuff if configured to do so
        if hasattr(args, "spi_connection") and args.spi_connection is not None:
            if esp.CHIP_NAME != "ESP32":
                raise FatalError("Chip %s does not support --spi-connection option." % esp.CHIP_NAME)
            _print("Configuring SPI flash mode...")
            esp.flash_spi_attach(args.spi_connection)
        elif args.no_stub:
            _print("Enabling default SPI flash mode...")
            # ROM loader doesn't enable flash unless we explicitly do it
            esp.flash_spi_attach(0)

        if hasattr(args, "flash_size"):
            _print("Configuring flash size...")
        ...
        if operation_func == load_ram:
            ...
            _print('Exiting immediately.')
        elif args.after == 'hard_reset':
            _print('Hard resetting via RTS pin...')
            esp.hard_reset()
        elif args.after == 'soft_reset':
            _print('Soft resetting...')
            # flash_finish will trigger a soft reset
            esp.soft_reset(False)
        else:
            _print('Staying in bootloader.')
            if esp.IS_STUB:
...
def expand_file_arguments():
    ...
    for arg in sys.argv:
        if arg.startswith("@"):
            ...
            new_args.append(arg)
    if expanded:
        _print("esptool.py %s" % (" ".join(new_args[1:])))
        sys.argv = new_args
...
class FlashSizeAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        try:
            ...
            _print("WARNING: Flash size arguments in megabits like '%s' are deprecated." % (values[0]))
            _print("Please use the equivalent size '%s'." % (value))
            _print("Megabit arguments may be removed in a future release.")
        except KeyError:
            value = values[0]
        ...
        if value not in known_sizes:
            raise Exception(self, '%s is not a known flash size. Known sizes: %s' % (value, ", ".join(known_sizes.keys())))
...
class SpiConnectionAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, value, option_string=None):
        if value.upper() == "SPI":
            ...
            values = value.split(",")
            if len(values) != 5:
                raise Exception(self, '%s is not a valid list of comma-separate pin numbers. Must be 5 numbers - CLK,Q,D,HD,CS.' % value)
            try:
                values = tuple(int(v,0) for v in values)
            except ValueError:
                raise Exception(self, '%s is not a valid argument. All pins must be numeric values' % values)
            if any([v for v in values if v > 33 or v < 0]):
                raise Exception(self, 'Pin numbers must be in the range 0-33.')
            # encode the pin numbers as a 32-bit integer with packed 6-bit values, the same way ESP32 ROM takes them
            # TODO: make this less ESP32 ROM specific somehow...
            clk,q,d,hd,cs = values
            value = (hd << 24) | (cs << 18) | (d << 12) | (q << 6) | clk
        else:
            raise Exception(self, '%s is not a valid spi-connection value. ' +
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        for i in range(0,len(values),2):
            try:
                ...
                raise Exception(self,'Address "%s" must be a number' % values[i])
            try:
                argfile = open(values[i + 1], 'rb')
            except IOError as e:
                raise Exception(self, e)
            except IndexError:
                raise Exception(self,'Must be pairs of an address and the binary filename to write there')
            pairs.append((address, argfile))

        # Sort the addresses and check for overlapping
        end = 0
        for address, argfile in sorted(pairs, key=lambda tup: tup[0]):
            argfile.seek(0,2)  # seek to end
            size = argfile.tell()
            argfile.seek(0)
